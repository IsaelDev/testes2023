...
describe('music servic', ()=>{
    ...
    test("should update the music count correctly after adding a new music", () => {
        musicRepository.getQtMusics.mockReturnValueOnce(0); 
        const initialQtMusic = musicsService.getQtMusic();
        ...
        musicRepository.getQtMusics.mockReturnValueOnce(1); 
    ...
    test("Testa se push é chamado corretamente", () => {
        const musicRepository = require("../music.repository").default;
        const pushSpy = jest.spyOn(musicRepository, "addMusic");
        const music = "Some Music";
      
        musicRepository.addMusic(music);
      
        expect(pushSpy).toHaveBeenCalledWith(music);
        expect(musicRepository.getQtMusics()).toBe(1);
      });
  ...
  //teste17
  test('should remove a music correctly when there is only one music in the array', () => {
    const musicToRemove = { title: 'One Dance', artist: 'Drake' };
  
    const removeMusicSpy = jest.spyOn(musicRepository.default, 'removeMusic');
    const getQtMusicsSpy = jest.spyOn(musicRepository.default, 'getQtMusics').mockReturnValueOnce(0);
  
    musicsService.remove(musicToRemove);
  
    expect(removeMusicSpy).toHaveBeenCalledWith(musicToRemove);
    expect(removeMusicSpy).toHaveBeenCalledTimes(1);
    expect(getQtMusicsSpy).toHaveBeenCalledTimes(1);
    expect(getQtMusicsSpy).toHaveReturnedWith(0);
  });
